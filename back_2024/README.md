...
项目的 `config/config.toml` 是配置信息，目前后端与爬虫共用一个配置文件，有关爬虫的配置请参考其 README。

你需要先填写好配置信息才能成功运行本项目。一般来说你需要将其中的 MySQL 和 Redis 的信息填成自己本地的。还建议你将 `log` 中的 `filter_level` 设为 `debug`，将 `with_ansi` 设为 `true` 以方便调试。
...
*小Tip：像上面这种需要在本地修改配置文件但不需要同步到云端的，可以使用 `git update-index --skip-worktree filename` 来忽略本地更改。若pull时远程仓库修改了这个文件，Git会发现冲突并提醒你，防止冲掉本地的配置。使用`--no-skip-worktree`恢复跟踪。*
