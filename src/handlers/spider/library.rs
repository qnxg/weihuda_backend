...
use crate::entities::spider::library::{Borrowed, Borrowing, Finance, LibraryRes};
use crate::{
...
pub async fn get_library_handler(Extension(token): Extension<String>) -> AppResult {
    ...
    let (borrowed, borrowing, finance): (Vec<Borrowed>, Vec<Borrowing>, Vec<Finance>) = try_join!(
        spider_data("/library/history_loan", &params),
        spider_data("/library/current_loan", &params),
        spider_data("/library/finance", &params),
    )?;
    let res = LibraryRes { borrowed, borrowing, finance };
    Ok(res.into())
}
