...
impl Cache {
    ...
    pub fn get(&self, key: &str) -> Option<String> {
        ...
        let last_request = self.last_request.get(key, &last_request_guard).cloned();
        self.last_request
            .insert(key.to_string(), Local::now().naive_local(), &last_request_guard);
        ...
        let last_source = self.last_source.get(key, &last_source_guard).cloned();
    ...
    pub fn increment_counter(&self, key: &str) -> Option<u32> {
        ...
        if let Some(counter) = self.counters.get(key, &counters_guard) {
            ...
            self.counters.insert(key.to_string(), new_counter, &counters_guard);
            Some(new_counter)
        } else {
            None
        }
...
pub async fn cache_middleware(
    ...
    if CACHE_PATHS.contains(&path) {
        let jwt = request.headers().get("Authorization").map(|t| t.to_str().unwrap());
